...
import com.gz.common.constant.GenConstant;
import com.gz.common.util.DateUtil;
import com.gz.common.util.StringUtil;
import com.gz.generator.domain.entity.GenTable;
import com.gz.generator.domain.entity.GenTableColumn;
...
public class VelocityUtils {
    ...
    public static VelocityContext prepareContext(GenTable genTable) {
        ...
        if (GenConstant.TPL_TREE.equals(tplCategory)) {
            setTreeVelocityContext(velocityContext, genTable);
        }
        if (GenConstant.TPL_SUB.equals(tplCategory)) {
    ...
    public static void setTreeVelocityContext(VelocityContext context, GenTable genTable) {
        ...
        if (paramsObj.containsKey(GenConstant.TREE_PARENT_CODE)) {
            context.put("tree_parent_code", paramsObj.getString(GenConstant.TREE_PARENT_CODE));
        }
        if (paramsObj.containsKey(GenConstant.TREE_NAME)) {
            context.put("tree_name", paramsObj.getString(GenConstant.TREE_NAME));
        }
    }
    ...
    public static List<String> getTemplateList(String tplCategory, String tplWebType) {
        ...
        if (GenConstant.TPL_CRUD.equals(tplCategory)) {
            templates.add(useWebType + "/index.vue.vm");
        } else if (GenConstant.TPL_TREE.equals(tplCategory)) {
            templates.add(useWebType + "/index-tree.vue.vm");
        } else if (GenConstant.TPL_SUB.equals(tplCategory)) {
            templates.add(useWebType + "/index.vue.vm");
    ...
    public static String getFileName(String template, GenTable genTable) {
        ...
        if (template.contains("domain.java.vm")) {
            fileName = StringUtil.format("{}/domain/entity/{}DO.java", javaPath, className);
        }
        if (template.contains("vo.java.vm")) {
            fileName = StringUtil.format("{}/domain/vo/{}VO.java", javaPath, className);
        }
        if (template.contains("convert.java.vm")) {
            fileName = StringUtil.format("{}/domain/convert/{}Convert.java", javaPath, className);
        }
        if (template.contains("sub-domain.java.vm") && StringUtil.equals(GenConstant.TPL_SUB, genTable.getTplCategory())) {
    ...
    public static HashSet<String> getImportList(GenTable genTable) {
        ...
        for (GenTableColumn column : columns) {
            if (!column.isSuperColumn() && GenConstant.TYPE_DATE.equals(column.getJavaType())) {
                importList.add("java.util.Date");
                importList.add("com.fasterxml.jackson.annotation.JsonFormat");
            } else if (!column.isSuperColumn() && GenConstant.TYPE_BIGDECIMAL.equals(column.getJavaType())) {
    ...
    public static void addDicts(Set<String> dicts, List<GenTableColumn> columns) {
        for (GenTableColumn column : columns) {
            if (!column.isSuperColumn() && StringUtil.isNotEmpty(column.getDictType()) && StringUtil.equalsAny(
                    column.getHtmlType(),
                    new String[]{GenConstant.HTML_SELECT, GenConstant.HTML_RADIO, GenConstant.HTML_CHECKBOX})) {
    ...
    public static String getParentMenuId(JSONObject paramsObj) {
        if (StringUtil.isNotEmpty(paramsObj) && paramsObj.containsKey(GenConstant.PARENT_MENU_ID)
                && StringUtil.isNotEmpty(paramsObj.getString(GenConstant.PARENT_MENU_ID))) {
            return paramsObj.getString(GenConstant.PARENT_MENU_ID);
        }
    ...
    public static String getTreecode(JSONObject paramsObj) {
        if (paramsObj.containsKey(GenConstant.TREE_CODE)) {
            return StringUtil.toCamelCase(paramsObj.getString(GenConstant.TREE_CODE));
        }
    ...
    public static String getTreeParentCode(JSONObject paramsObj) {
        if (paramsObj.containsKey(GenConstant.TREE_PARENT_CODE)) {
            return StringUtil.toCamelCase(paramsObj.getString(GenConstant.TREE_PARENT_CODE));
        }
        return StringUtil.EMPTY;
    }
    ...
    public static String getTreeName(JSONObject paramsObj) {
        if (paramsObj.containsKey(GenConstant.TREE_NAME)) {
            return StringUtil.toCamelCase(paramsObj.getString(GenConstant.TREE_NAME));
        }
        return StringUtil.EMPTY;
    }
    ...
    public static int getExpandColumn(GenTable genTable) {
        String options = genTable.getOptions();
        JSONObject paramsObj = JSON.parseObject(options);
        String treeName = paramsObj.getString(GenConstant.TREE_NAME);
        int num = 0;
