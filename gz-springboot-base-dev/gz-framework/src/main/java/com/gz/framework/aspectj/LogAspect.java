...
import com.gz.common.enums.HttpMethodEnum;
import com.gz.common.enums.StatusEnum;
import com.gz.framework.security.filter.PropertyPreExcludeFilter;
...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            operLog.setStatus(StatusEnum._0.getIntCode());
            // 请求的地址
            String ip = IpUtil.getIpAddr();
            operLog.setOperIp(ip);
            operLog.setOperUrl(StringUtil.substring(ServletUtil.getRequest().getRequestURI(), 0, 255));
            if (loginUserDTO != null) {
                operLog.setOperName(loginUserDTO.getUsername());
                SysUser currentUser = loginUserDTO.getUser();
                if (StringUtil.isNotNull(currentUser) && StringUtil.isNotNull(currentUser.getDept())) {
                    operLog.setDeptName(currentUser.getDept().getDeptName());
                }
            }

            if (e != null) {
                operLog.setStatus(StatusEnum._1.getIntCode());
