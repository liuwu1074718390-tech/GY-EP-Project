...
public class SysDeptController extends BaseController {
    ...
    public AjaxResult excludeChild(@PathVariable(value = "deptId", required = false) Long deptId) {
        ...
        depts.removeIf(d -> d.getDeptId().intValue() == deptId || ArrayUtils.contains(StringUtil.split(d.getAncestors(), ","), deptId + ""));
        return success(depts);
    }
    ...
    @Log(title = "部门管理", businessType = BizTypeEnum.INSERT)
    @PostMapping
    ...
    @Log(title = "部门管理", businessType = BizTypeEnum.UPDATE)
    @PutMapping
    public AjaxResult edit(@Validated @RequestBody SysDept dept) {
        ...
        } else if (StringUtil.equals(UserConstant.DEPT_DISABLE, dept.getStatus()) && deptService.selectNormalChildrenDeptById(deptId) > 0) {
    ...
    @Log(title = "部门管理", businessType = BizTypeEnum.DELETE)
