...
public class SysMenuController extends BaseController {
    ...
    @Log(title = "菜单管理", businessType = BizTypeEnum.INSERT)
    @PostMapping
    public AjaxResult add(@Validated @RequestBody SysMenu menu) {
        if (!menuService.checkMenuNameUnique(menu)) {
            return error("新增菜单'" + menu.getMenuName() + "'失败，菜单名称已存在");
        } else if (UserConstant.YES_FRAME.equals(menu.getIsFrame()) && !StringUtil.ishttp(menu.getPath())) {
    ...
    @Log(title = "菜单管理", businessType = BizTypeEnum.UPDATE)
    @PutMapping
    ...
    @Log(title = "菜单管理", businessType = BizTypeEnum.DELETE)
